...
module.exports = function getValidators(params) {
  ...
  const paramTypes = params.reduce((types, param) => {
    ...
    // Make name lowercase if header
    if (key === 'header') name = name.toLowerCase()
    types[key] = types[key] || {
      $async: true,
      type: 'object',
      properties: {},
  ...
  collections.forEach((param) => {
    ...
    if (property.default) {
      paramTypes[param.in].properties[param.name].default = property.default
    }
  })
  ...
  if (paramTypes.formData) validators.body = ajv.compile(paramTypes.formData)
  if (paramTypes.body) validators.body = ajv.compile(paramTypes.body)
  if (!validators.body) validators.body = ajv.compile(STRIP_EXTRA)
  return validators
}
